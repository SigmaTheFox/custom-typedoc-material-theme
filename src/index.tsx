...
export function load(app: Application) {
	...
	app.renderer.hooks.on('head.begin', event => (
		<>
			<script>
				<JSX.Raw
	...
	app.renderer.hooks.on('head.end', event => (
		<>
			<style>
				<JSX.Raw
					html={getThemeCSSProperties(app.options.getValue('themeColor') as string)}
				/>
			</style>
			<link rel="stylesheet" href={event.relativeURL('assets/material-style.css')} />
		</>
	));
